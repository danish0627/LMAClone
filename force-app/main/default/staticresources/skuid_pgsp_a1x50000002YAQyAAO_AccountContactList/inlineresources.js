...
/*(function(skuid){
	var $ = skuid.$;
	$(document.body).one('pageload',function(){
var acc = document.getElementsByClassName("cc-accountContact-header-accordion");
var i;
...
/*
    ** PageLoad Snippet to display account's contact lists as per event edition (opportunity).
...
(function(skuid){
	...
	$(document.body).one('pageload',function(){
	 //$(window).on('load',function(){
        ...
        var rowCU = modelCU.getFirstRow();
        var accId = rowCU.Contact.AccountId;
    	var modelB = skuid.model.getModel('AccountContactUI');
    	var modelBooth = skuid.model.getModel('ExpocadBoothUI');
    	
	    var modelP = skuid.model.getModel('PriContactUI');
        var modelI = skuid.model.getModel('InvContactUI');
        var modelO1 = skuid.model.getModel('Op1ContactUI');
    	
    	
       /* Visualforce.remoting.Manager.invokeAction(
            '{!$RemoteAction.EventEditionAccountContact.EventEditionAccountContact}',
            eId,accId, function(result, event){
            var a = result;
            console.log("amish"+a);
        ...
        var result = sforce.apex.execute('EventEditionAccountContact','EventEditionAccountContact', { eId:eId, accId: accId});
        console.log(result);
        var resultBooth = sforce.apex.execute('EventEditionAccountContact','EventEditionAccountEventBooth', { eId:eId, accId: accId});
        console.log('resultBooth  resultBooth'+resultBooth);
    	var conList = JSON.parse(result);
    	console.log(conList);
    	var boothList = JSON.parse(resultBooth);
    	//var rowsToInsert = {}
    	// Populating UI Model with data capture from salesforce for product
     	if(conList)
        {
            $.each(conList, function( index,value) {
            var contactList = modelB.createRow({ 
                    additionalConditions: [
                        { field: 'ContactId', value: value.ContactId},
                       //{ field: 'FirstName', value: value.FirstName},
                        //{ field: 'LastName', value: value.LastName},
                        //{ field: 'ContactType', value: value.ContactType},
                        { field: 'Name', value: value.name},
                        { field: 'Type', value: value.type },
                    ],doAppend: true});
        ...
        if(boothList)
        {
            $.each(boothList, function( index,value) {
            var boothLists = modelBooth.createRow({ 
                    additionalConditions: [
                        { field: 'BoothId', value: value.boothId},
                        { field: 'BoothName', value: value.boothName},
                        { field: 'BoothContactId', value: value.boothContactId},
                        { field: 'BoothContactName', value: value.boothContactName},
                    ],doAppend: true});
...
skuid.snippet.register('PrimarySnippet',function(args) {var params = arguments[0],
	$ = skuid.$;
    var modelB = skuid.model.getModel('AccountContactUI');
    for (var i=0;i<=modelB.data.length();i++)
    console.log('amish'+modelB.data[i].Type);
    {
        if ((modelB.data[i].Type).contains('Pri'))
        {
            return false;
...
skuid.snippet.register('CreateContactRecords',function(args) {var params = arguments[0],
	$ = skuid.$;
	
    var modelB = skuid.$M('AccountContactUI');
    var modelP = skuid.model.getModel('PriContactUI');
    var modelI = skuid.model.getModel('InvContactUI');
    var modelO1 = skuid.model.getModel('Op1ContactUI');
    var modelO2 = skuid.model.getModel('Op2ContactUI');
    var modelO3 = skuid.model.getModel('Op3ContactUI');
    var modelO4 = skuid.model.getModel('Op4ContactUI');
    var modelO5 = skuid.model.getModel('Op5ContactUI');
    var modelH = skuid.model.getModel('HealthContactUI');
    
    var modelBLebgth= skuid.model.getModel('AccountContactUI').data.length;
    if (modelBLebgth>0)
    {
        for (var i=0;i<=modelBLebgth;i++)
        {
            if ((modelB.data[i].Type).includes("Pri"))
            {
                var modelPcontactList = modelP.createRow({ 
                        additionalConditions: [
                            { field: 'ContactId', value: modelB.data[i].ContactId},
                            { field: 'Name', value: modelB.data[i].Name},
                            { field: 'Type', value: modelB.data[i].Type },
                        ],doAppend: true});
            
                modelP.save();
            }
            if ((modelB.data[i].Type).includes('Inv'))
            {
                 var modelIcontactList = modelI.createRow({ 
                        additionalConditions: [
                            { field: 'ContactId', value: modelB.data[i].ContactId},
                            { field: 'Name', value: modelB.data[i].Name},
                            { field: 'Type', value: modelB.data[i].Type },
            ...
            if ((modelB.data[i].Type).includes('Hlt'))
            {
                 var modelHltcontactList = modelH.createRow({ 
                        additionalConditions: [
                            { field: 'ContactId', value: modelB.data[i].ContactId},
                            { field: 'Name', value: modelB.data[i].Name},
                            { field: 'Type', value: modelB.data[i].Type },
            ...
            if ((modelB.data[i].Type).includes('Op1'))
            {
                 var modelO1contactList = modelO1.createRow({ 
                        additionalConditions: [
                            { field: 'ContactId', value: modelB.data[i].ContactId},
                            { field: 'Name', value: modelB.data[i].Name},
                            { field: 'Type', value: modelB.data[i].Type },
            ...
            if ((modelB.data[i].Type).includes('Op2'))
            {
                 var modelO2contactList = modelO2.createRow({ 
                        additionalConditions: [
                            { field: 'ContactId', value: modelB.data[i].ContactId},
                            { field: 'Name', value: modelB.data[i].Name},
                            { field: 'Type', value: modelB.data[i].Type },
            ...
            if ((modelB.data[i].Type).includes('Op3'))
            {
                 var modelO3contactList = modelO3.createRow({ 
                        additionalConditions: [
                            { field: 'ContactId', value: modelB.data[i].ContactId},
                            { field: 'Name', value: modelB.data[i].Name},
                            { field: 'Type', value: modelB.data[i].Type },
            ...
            if ((modelB.data[i].Type).includes('Op4'))
            {
                 var modelO4contactList = modelO4.createRow({ 
                        additionalConditions: [
                            { field: 'ContactId', value: modelB.data[i].ContactId},
                            { field: 'Name', value: modelB.data[i].Name},
                            { field: 'Type', value: modelB.data[i].Type },
            ...
            if ((modelB.data[i].Type).includes('Op5'))
            {
                 var modelO5contactList = modelO5.createRow({ 
                        additionalConditions: [
                            { field: 'ContactId', value: modelB.data[i].ContactId},
                            { field: 'Name', value: modelB.data[i].Name},
                            { field: 'Type', value: modelB.data[i].Type },
